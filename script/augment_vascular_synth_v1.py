...
def add_some_dill(ilab, dic_map):
    ...
    def dill_GM(ilab):
        ...
        if nb_iter>2:
            within_label.append(dic_map['Vas_brain'])
            out_prefix = f'{out_prefix}Vas_it{nb_iter}'
        else:
...
def generate_one_suj(label_file, label_csv, fout, transfo_list):
    ...
    foimg, folab = addprefixtofilenames(fout,'Sim_')[0], addprefixtofilenames(fout,'Lab_')[0]
    ...
    suj_label, suj_label_4D_not_used = pool_remap(i_lab_target, pooling_size=3, ensure_multiple=6, tmap=None, islabel=True)
    ...
    folab = folab[:-7] + out_prefix + transfo_name + '.nii.gz'
    foimg = foimg[:-7] + out_prefix + transfo_name + '.nii.gz'
    suj_label.save(folab);
...
output_dir = dirout + '/generate_noaffine/'
nb_example_per_subject = 100

for ngen in range(nb_example_per_subject):
    print(f'Pass {ngen+1}')
    for k, (label_file, label_csv) in enumerate(zip(fins,flabels) ) :
        sujname = f'Suj_{k:02}'
